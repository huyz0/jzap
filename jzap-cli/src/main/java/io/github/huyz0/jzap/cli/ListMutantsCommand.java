...
import io.github.huyz0.jzap.core.Reduction;
...
final class ListMutantsCommand implements Callable<Integer> {
    ...
    public Integer call() {
        ...
        MutationEngine engine = new MutationEngine(
                Mutators.resolve(model.scope().mutators()), MutantFilters.from(model.scope()));
        ...
        Reduction dropped = engine.reduction();
        if (dropped.total() > 0 || !model.scope().enabledFilters().isEmpty()) {
            System.err.println("jzap: filters dropped " + dropped.total() + " mutant(s): "
                    + dropped.equivalent() + " equivalent to the original, "
                    + dropped.duplicate() + " duplicates, "
                    + dropped.arid() + " in code that only reports, "
                    + dropped.onePerLine() + " beyond one per line, "
                    + dropped.kotlinJunk() + " generated by the Kotlin compiler");
        }
